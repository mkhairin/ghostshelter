...
# Dictionary untuk memetakan port ke layanan
port_services = {
    20: 'FTP Data',
    21: 'FTP',
    22: 'SSH',
...
# Fungsi hidden files
# Daftar nama file yang sering tersembunyi
hidden_files = [
    '.env', '.git/config', '.gitignore', '.htaccess', '.svn/entries', '.idea/',
    'admin/', 'backup/', 'config/', 'db/', 'logs/', 'temp/', 'uploads/',

    # Menambahkan lebih banyak file dan direktori tersembunyi
...
def check_hidden_files(url):
    for file in hidden_files:
        ...
        try:
            ...
            if status_code == 200:
                print(
                    f"{Fore.GREEN}File found (Status {status_code}): {test_url}{Style.RESET_ALL}")
            elif status_code == 403:
                print(
                    f"{Fore.RED}Access denied (Status {status_code}): {test_url}{Style.RESET_ALL}")
            elif status_code == 404:
                print(
                    f"File not found (Status {status_code}): {test_url}")
            else:
                print(f"Status {status_code} untuk {test_url}")
        except requests.RequestException as e:
...
if choice == "1":
    ...
    url = input(
        "Enter the URL to scan (e.g., http://example.com): ")
    ...
    if host:
        try:
            ...
            # Daftar port umum yang akan dipindai
            common_ports = list(port_services.keys())

            print(f"Scanning host: {host} ({host_ip})")
            ...
            print(f"{Fore.RED}Invalid host: {host}{Style.RESET_ALL}")
    else:
    ...
    check_hidden_files(website_url)
else:
    print("Invalid selection. Please enter the available options.")
